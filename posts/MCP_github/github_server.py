...
# Generar un par de claves RSA para desarrollo y testing
print("🔐 Generando par de claves RSA para autenticación...")
key_pair = RSAKeyPair.generate()

# Configurar el proveedor de autenticación Bearer
auth_provider = BearerAuthProvider(
    public_key=key_pair.public_key,
    issuer="https://github-mcp.maxfn.dev",
    audience="github-mcp-server",
    required_scopes=["github:read"]  # Scope global requerido para todos los requests
)
...
development_token = key_pair.create_token(
    ...
    expires_in_seconds=3600 * 24  # Token válido por 24 horas
)

print(f"🎫 Token de desarrollo generado:")
print(f"   {development_token}")
print("💡 Usa este token en el cliente para autenticarte")
print("-" * 60)

# Create FastMCP server con autenticación
mcp = FastMCP(
    name="GitHubMCP",
    instructions="This server provides tools, resources and prompts to interact with the GitHub API.",
    include_tags={"public"},
    exclude_tags={"first_issue"},
    auth=auth_provider  # Añadir autenticación al servidor
)
...
async def list_repository_issues(owner: str, repo_name: str, ctx: Context, user_id: int = USER_ID) -> list[dict]:
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            # Obtener información del token de acceso autenticado
            try:
...
def generate_issues_prompt(owner: str, repo_name: str) -> str:
    """
    Generates a prompt to generate issues for a given repository.
    """
    return f"""
    Can you give me a list of issues for the repository {owner}/{repo_name}?
    """
